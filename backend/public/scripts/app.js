...
// Volumen elements
const footerVolumeBtn = document.getElementById("footer-volume-btn");
const footerVolumeSlider = document.getElementById("footer-volume-slider");
const footerVolumeDisplay = document.getElementById("footer-volume-display");

// Cola de reproducción elements
const footerQueueBtn = document.getElementById("footer-queue-btn");
...
function updateVolumeIcon(volume) {
    const svg = footerVolumeBtn.querySelector('svg');
...
function setVolume(value) {
    ...
    footerVolumeSlider.value = value;
    footerVolumeDisplay.textContent = value;
    updateVolumeIcon(value);
    updateRange(footerVolumeSlider); // 👈 esto faltaba
...
function toggleMute() {
    if (isMuted) {
        ...
        if (parseInt(footerVolumeSlider.value) > 0) {
            lastVolume = parseInt(footerVolumeSlider.value);
        }
        setVolume(0);
        isMuted = true;
    }
...
footerVolumeBtn.addEventListener('click', toggleMute);
...
function toggleQueue() {
    ...
    if (queueOpen) {
        ...
        queueModal.style.left = 'auto';
        queueModal.style.right = `${window.innerWidth - footerRect.right}px`;
        queueModal.style.bottom = `calc(40px + 80px + 1rem)`;
        
        queueModal.classList.add('show');
        footerQueueBtn.classList.add('active');
        updateQueueModal();
    } else {
        queueModal.classList.remove('show');
        footerQueueBtn.classList.remove('active');
    }
}

// Reemplaza la función updateQueueModal completa:
...
function updateQueueModal() {
    ...
    if (!currentContext) {
        queueModalList.innerHTML = `
            <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 3rem 1rem; color: rgba(255, 255, 255, 0.4);">
                ...
                <p style="font-size: 14px; margin: 0;">No hay canciones en cola</p>
            </div>
        `;
    ...
    // En lugar de usar currentPlaylist, reconstruir desde la BD según el contexto
    const tx = db.transaction(["songs", "favorites"], "readonly");
    ...
    } else if (currentContext === "playlist" && currentPlaylistId) {

    const playlistTx = db.transaction(
        ["playlists", "songs"],
        "readonly"
    );

    const playlistStore = playlistTx.objectStore("playlists");
    const songStore = playlistTx.objectStore("songs");

    const playlistReq = playlistStore.get(currentPlaylistId);

    playlistReq.onsuccess = () => {

        const playlist = playlistReq.result;

        if (!playlist) {
            renderQueueFromSongs([]);
            return;
        }

        const songIds = playlist.songsId || [];
        ...
        songIds.forEach(id => {
            ...
            const req = songStore.get(id);

            req.onsuccess = () => {

                if (req.result) {
                    songs.push(req.result);
                }
                ...
                if (loaded === songIds.length) {
                    ...
                    renderQueueFromSongs(orderedSongs);
                }
            };
        });
    };
...
// Nueva función auxiliar para renderizar la cola
function renderQueueFromSongs(songs) {
    queueModalList.innerHTML = '';
    ...
    // Actualizar currentPlaylist con los IDs
    currentPlaylist = songs.map(song => song.id);
    
    queueCount.textContent = `${songs.length} ${songs.length === 1 ? 'canción' : 'canciones'}`;
    
    songs.forEach((song, index) => {
        const isCurrent = song.id === currentSongId;
        const queueItem = document.createElement('div');
        queueItem.classList.add('queue-item');
        if (isCurrent) queueItem.classList.add('current');
        ...
        queueItem.innerHTML = `
            ...
            <img src="${song.picture}" class="queue-item-cover" alt="">
        ...
        queueModalList.appendChild(queueItem);
    });
}

// Event listeners para cola
footerQueueBtn.addEventListener('click', toggleQueue);
queueModalClose.addEventListener('click', toggleQueue);

// Cerrar modal al hacer clic fuera
document.addEventListener('click', (e) => {
    if (queueOpen && 
        !queueModal.contains(e.target) && 
        !footerQueueBtn.contains(e.target)) {
        toggleQueue();
    }
});
...
function loadAndPlaySong(song) {
    ...
    currentTitle = song.title || "Sin titulo";
    currentArtist = song.artist || "Artista desconocido";
    ...
    const pictureUrl = song.picture || "assets/default.jpeg";

    albumPortrait.src = pictureUrl;
    footerAlbumPortrait.src = pictureUrl;

    if (pictureUrl !== "assets/default.jpeg") {
    ...
    let url = URL.createObjectURL(song.file);
...
async function loadAndSaveSong() {
    ...
    // 👇 AGREGAR ESTO
    setTimeout(() => {
        if (queueOpen && currentContext === "saved") {
            updateQueueModal();
        }
    }, 100);
}
...
// ============================================
// CARGAR CANCIONES DE LA BASE DE DATOS
// ============================================
    ...
    song_items.forEach(song_item => {
        ...
        if (isSongList || isFavoriteList) {
            btn_favorite.addEventListener("click", () => {
                ...
                // En el event listener de btn_favorite, dentro de tx.oncomplete
                tx.oncomplete = () => {
                    loadFavoriteSongs();
                    
                    // 👇 AGREGAR ESTO
                    if (queueOpen && currentContext === "favorites") {
                        updateQueueModal();
                    }
                };
...
function buildCurrentPlaylist() {
    const visibleSection = Array.from(document.querySelectorAll('section')).find(s => s.style.display === "flex");
    ...
    // Actualizar cola si está abierta
    if (queueOpen) {
        updateQueueModal();
    }
}
...
// Variables nuevas — agrégalas junto a las otras globales
let lyricsCache = new Map();
let currentFetchController = null;

async function obtenerLetra(trackName, trackArtist) {
    ultimaCancion = { trackName, trackArtist };
    ...
    if (currentSong === "Sin reproducir") {
        mostrarEstadoLyrics("idle", "Nada reproduciéndose", "Selecciona una canción para ver las letras.");
        return;
    }

    const cacheKey = `${trackName}||${trackArtist}`;

    // Si ya está en caché, mostrar inmediatamente
    if (lyricsCache.has(cacheKey)) {
        const cached = lyricsCache.get(cacheKey);
        if (cached === null) {
            mostrarEstadoLyrics("no-lyrics", "Sin letras disponibles", "No encontramos letras sincronizadas para esta canción.");
        } else {
            letras = cached;
            indiceActual = -1;
    ...
    mostrarEstadoLyrics("loading", "Cargando letras...", "Estamos buscando letras sincronizadas.");
    ...
    try {
        ...
        lyricsCache.set(cacheKey, parsed); // guardar en caché
        letras = parsed;
        indiceActual = 0;
        mostrarLetras(letras);

    } catch (error) {
        if (error.name === "AbortError") return; // fetch cancelado, ignorar
        mostrarEstadoLyrics("offline", "Sin conexión", "Revisa tu conexión a internet o inténtalo más tarde.");
    } finally {
        currentFetchController = null;
    }
}
...
function parseLRC(lrc) {
    return lrc.split("\n").map(line => {
        ...
        return {
            tiempo: parseFloat(match[1]) * 60 + parseFloat(match[2]),
            texto: match[3].trim()
        };
    }).filter(Boolean);
}
...
function syncLyrics(tiempoActual) {
    ...
    // Activar línea 0 cuando el tiempo llegue a ella
    if (nuevoIndice === 0 && tiempoActual < letras[0].tiempo) {
        if (indiceActual !== -1) {
            indiceActual = -1;
...
const buscarResults = document.getElementById('buscar-results');
...
function showLoadingState() {
    buscarResults.innerHTML = `
        <div class="buscar-loading">
            <svg width="40" height="40" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M12 3V6M12 18V21M21 12H18M6 12H3M18.364 18.364L16.243 16.243M18.364 5.636L16.243 7.757M5.636 18.364L7.757 16.243M5.636 5.636L7.757 7.757" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
            </svg>
        </div>
    `;
}
...
function showEmptyState() {
    buscarResults.innerHTML = `
        <div class="buscar-empty-state">
            ...
            <span>Encuentra cualquier canción o artista</span>
        </div>
    `;
}
...
function showNoResults(query) {
    buscarResults.innerHTML = `
        <div class="buscar-empty-state">
            ...
            <span>Intenta con otro artista o canción</span>
        </div>
    `;
}
...
function parseDuration(duration) {
    // Convertir formato ISO 8601 (PT4M33S) a segundos
...
function displayResults(videos) {
    const resultsHTML = videos.map(video => {
        ...
        return `
            <div class="buscar-result-item" data-track-id="${videoId}">
                <img src="${thumbnail}" alt="${escapeHtml(title)}" class="buscar-result-cover">

                <div class="buscar-result-info">
                    <div class="buscar-result-title">${escapeHtml(title)}</div>
                    <div class="buscar-result-artist">${escapeHtml(channel)}</div>
                </div>

                <span class="buscar-result-duration">${formatTime(duration)}</span>

                <button class="buscar-result-download-btn"
    ...
    buscarResults.innerHTML = `<div class="buscar-results-list">${resultsHTML}</div>`;

    document.querySelectorAll('.buscar-result-download-btn').forEach(btn => {
        btn.addEventListener('click', async (e) => {
            e.preventDefault();
            e.stopPropagation();

            const trackData = JSON.parse(btn.getAttribute('data-track'));
            await downloadAndSaveTrack(trackData, btn);
        });
    });
}

async function downloadAndSaveTrack(trackData, button) {
    ...
    try {
        // 1. Pedir mp3 al backend
        const response = await fetch("http://localhost:3000/download", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ url: trackData.youtubeUrl })
        });

        if (!response.ok) throw new Error("Error descargando canción");

        const data = await response.json();

        // 2. Convertir base64 a blob/file
        ...
        // 4. ✨ NUEVO: Pedir metadatos limpios al backend (Groq + MusicBrainz)
        let songTitle  = trackData.name;
        let songArtist = trackData.artist_name;
        let pictureBase64 = null;

        try {
            const metaRes = await fetch("http://localhost:3000/metadata", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
            ...
            if (metaRes.ok) {
                ...
                songTitle     = meta.title  || songTitle;
                songArtist    = meta.artist || songArtist;
                pictureBase64 = meta.cover  || null;
            }
        } catch (err) {
            console.warn("No se pudieron obtener metadatos:", err.message);
        }

        // 5. Fallback: si MusicBrainz no trajo cover, usar thumbnail de YouTube
        if (!pictureBase64) {
            try {
                pictureBase64 = await imageToBase64(trackData.album_image);
            } catch {
                pictureBase64 = null;
            }
        }

        // 6. Crear objeto canción y guardar en IndexedDB
        const song = {
            title: songTitle,
            artist: songArtist,
            isFavorite: false,
        ...
        saveAudio(song, file, pictureBase64);

        // 7. Limpiar memoria
        URL.revokeObjectURL(audioUrl);

        // 8. Recargar lista y cola
        loadSavedSongs();
        setTimeout(() => {
            if (queueOpen && currentContext === "saved") {
                updateQueueModal();
